...
var Calc = {
    onLoad: function(obj) {
        ...
        $.ajax({
        	url: "/api/materials/" + $("#name").text(),
        	type: "get",
        	success: function(resp) {
        		var mtrl = JSON.parse(resp);
        		$("#total").append([
        		        "<hr><div class='row'>",
        			        "<div class='col-md-2'>",
        				        "<img class='img-responsive' src='" + mtrl.thumbs[0] + "'>",
        			        "</div>",
        			        "<div class='col-md-5'>",
        				        "<h3 class='text-left'>" + mtrl.name + "</h3>",
        			        "</div>",
        			        "<div class='col-md-3'>",
        			        	"<input type='number' class='form-control' value='1' style='width:50%; display: inline'> шт. <hr>",
        			            "<button data-material='" + mtrl.name + "' data-price='" + Math.ceil( mtrl.price * vol ) + "' class='btn btn-xs btn-primary order'>Добавить к заказу</button>",
        			        "</div>",
        			        "<div class='col-md-2'>",
        				        "<p>" + Math.ceil( mtrl.price * vol ) + " руб.</p>",
        			        "</div>",
        		        "</div>"].join(''));
